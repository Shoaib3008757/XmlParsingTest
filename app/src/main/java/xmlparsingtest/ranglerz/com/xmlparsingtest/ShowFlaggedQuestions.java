...
/**
 * Created by User-10 on 24-Oct-16.
 */
public class ShowFlaggedQuestions extends AppCompatActivity implements TextToSpeech.OnInitListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "Random Question size is " + Splash.randomQuestions.size());
    ...
    public void displayQuestionAndAnswer()
    {
        ...
        Log.d(TAG, "Question id is " + Splash.randomQuestions.get(currentFlagQuestionNumber).getQuestionId());
        Log.d(TAG, "Answers List size is " + Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().size());



        // this is use to show Images for Questions..
        if(!Splash.randomQuestions.get(currentFlagQuestionNumber).getQuestionImage().isEmpty()) {

            // get input stream
            InputStream ims = null;
            try {
                ims = getAssets().open("images/"+Splash.randomQuestions.get(currentFlagQuestionNumber).getQuestionImage().toString().trim());
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        if( Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(0).getAnswerText().isEmpty()) {
            ...
            questionText.setText(Splash.randomQuestions.get(currentFlagQuestionNumber).getQuestionText().toString());

            text = Splash.randomQuestions.get(currentFlagQuestionNumber).getQuestionText().toString();
            speackButtonClickListener(text);

            questionCategory.setText(Splash.randomQuestions.get(currentFlagQuestionNumber).getTopic().toString());

            correctImage = Splash.randomQuestions.get(currentFlagQuestionNumber).getCorrectImage();

            imageName = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(0).getImageName().toString().trim();
            Log.d(TAG, "ImageName is" + imageName);
            imageName1 = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(1).getImageName().toString().trim();
            Log.d(TAG,"ImageName is"+imageName1);
            imageName2 = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(2).getImageName().toString().trim();
            Log.d(TAG, "ImageName is"+imageName2);
            imageName3 = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(3).getImageName().toString().trim();
        ...
        correctAnswer =  Splash.randomQuestions.get(currentFlagQuestionNumber).getCorrectAnswer().toString().trim();
        ...
        questionCategory.setText(Splash.randomQuestions.get(currentFlagQuestionNumber).getTopic().toString());
        questionNumber.setText("Question " + questionNumberIncremented + " off "+Splash.randomQuestions.size());


        /// setting the answer of the questions...
        answer1.setText(Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(0).getAnswerText());
        textAnswer1 = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(0).getAnswerText().toString();
        ans1ButtonSpeech(textAnswer1);


        answer2.setText(Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(1).getAnswerText());
        textAnswer2 = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(1).getAnswerText().toString();
        ans2ButtonSpeech(textAnswer2);


        answer3.setText(Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(2).getAnswerText());
        textAnswer3 = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(2).getAnswerText().toString();
        ans3ButtonSpeech(textAnswer3);


        answer4.setText(Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(3).getAnswerText());
        textAnswer4 = Splash.randomQuestions.get(currentFlagQuestionNumber).getAnswerList().get(3).getAnswerText().toString();
        ...
        if(imageShowing)
        {
            ...
            if (answer1.isPressed()) {
                ...
                if (correctAnswer.equals(userAnswer)) {
                    ...
                    // also remove from the wrong list because we donot want user choice in both arrays...
                    userChoiceTakeTest.numberOfCorrectAnswers.put(currentFlagQuestionNumber, 1);
                    userChoiceTakeTest.numberOfWrongAnswers.remove(currentFlagQuestionNumber);
...
//
    ...
    public void ShowFlagDialog(){
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(ShowFlaggedQuestions.this);
            ...
            public void onClick(DialogInterface dialog, int id) {
                ...
                .setNegativeButton("Review Answers", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        Intent i = new Intent(ShowFlaggedQuestions.this, ReviewAnswers.class);
